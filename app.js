...
function normalizeLead(value) {
  ...
  const status = STATUSES.includes(lead.status) ? lead.status : "Lead";
...
function saveLeadFromForm(event) {
  ...
  const status = STATUSES.includes(els.leadStatus.value) ? els.leadStatus.value : "Lead";
...
function archiveCurrentLead() {
  ...
  state.leads = state.leads.map((lead) =>
    lead.id === id ? { ...lead, archived: true, timerSeconds: getLeadElapsed(lead), timerStartedAt: null } : lead
  );
  closeLeadDialog();
  saveState();
  renderAll();
...
function toggleJobTimer(id) {
  const lead = state.leads.find((item) => item.id === id);
  if (!lead) return;
  if (lead.status === "Completed" || lead.status === "Lost" || lead.archived) return;
