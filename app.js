const STATUSES = ["Lead", "Knocked", "Quoted", "Booked", "In Progress", "Completed", "Lost"];
  ...
  if (saved) {
    try {
      ...
      return {
        ...
        routeOrder: parsed.routeOrder || seedLeads.map((lead) => lead.id),
...
function renderDashboard() {
  ...
  const avgRevenue = completed.length ? completed.reduce((sum, lead) => sum + number(lead.price), 0) / completed.length : 0;
  const progress = Math.min(100, Math.round((todayRevenue / DAILY_GOAL) * 100));

  els.todayRevenue.textContent = currency(todayRevenue);
  els.weekRevenue.textContent = currency(weekRevenue);
  els.monthRevenue.textContent = currency(monthRevenue);
  els.avgRevenue.textContent = currency(avgRevenue);
  ...
  const todayJobs = activeLeads
    .filter((lead) => lead.scheduledDate === todayISO() && ["Booked", "In Progress", "Completed"].includes(lead.status))
...
function renderPipeline() {
  ...
  els.pipelineBoard.querySelectorAll(".lead-card").forEach((card) => {
...
function renderJobTimers() {
  const jobs = getActiveLeads().filter((lead) => ["Booked", "In Progress", "Completed"].includes(lead.status));
...
function bindCalendarDrag() {
  document.querySelectorAll(".calendar-job").forEach((job) => {
  ...
  document.querySelectorAll(".day-cell, .month-day").forEach((cell) => {
...
function openLeadDialog(id = "") {
  ...
  els.leadStatus.value = lead?.status || "Lead";
...
function saveLeadFromForm(event) {
  ...
  const status = els.leadStatus.value;
  const candidate = {
    id,
  ...
  const lead = {
    ...
    timerStartedAt: previous?.timerStartedAt || null,
  };
...
function archiveCurrentLead() {
  ...
  if (!id) return;
  state.leads = state.leads.map((lead) => (lead.id === id ? { ...lead, archived: true } : lead));
  closeLeadDialog();
  saveState();
  renderAll();
...
function adjustCounter(counter, step) {
  state.session[counter] = Math.max(0, state.session[counter] + step);
  saveState();
  renderSession();
}
...
function routeFromBooked() {
  const booked = getActiveLeads()
    .filter((lead) => ["Booked", "In Progress"].includes(lead.status))
...
function scheduleLead(id, date, hour) {
  ...
  lead.status = lead.status === "Lead" || lead.status === "Knocked" || lead.status === "Quoted" ? "Booked" : lead.status;
  saveState();
  renderAll();
...
function toggleJobTimer(id) {
  ...
  if (lead.timerStartedAt) {
    ...
    if (lead.status === "Booked") lead.status = "In Progress";
...
function downloadLeadCalendar(lead) {
  if (!hasScheduledTime(lead)) {
    showToast("Add a scheduled date and time first");
    return;
  }
  const blob = new Blob([icsForLead(lead)], { type: "text/calendar;charset=utf-8" });
  const url = URL.createObjectURL(blob);
  const link = document.createElement("a");
  link.href = url;
...
function renderSyncState() {
  const saved = localStorage.getItem(STORAGE_KEY) ? "Saved locally" : "Offline-ready";
...
function getRouteStops(selectedZone) {
  const ids = new Set(state.routeOrder);
  const routeLeads = getActiveLeads()
    .filter((lead) => ["Booked", "In Progress", "Completed"].includes(lead.status))
    .filter((lead) => selectedZone === "All zones" || lead.zone === selectedZone)
    .sort((a, b) => {
      const indexA = state.routeOrder.indexOf(a.id);
      const indexB = state.routeOrder.indexOf(b.id);
...
function completedTodayCount() {
  return getActiveLeads().filter((lead) => lead.status === "Completed" && lead.completedAt && toISODate(new Date(lead.completedAt)) === todayISO()).length;
}

function getSessionElapsed() {
  if (!state.session.active || !state.session.startedAt) return state.session.elapsedSeconds;
  return state.session.elapsedSeconds + Math.floor((Date.now() - state.session.startedAt) / 1000);
}

function getLeadElapsed(lead) {
  if (!lead.timerStartedAt) return lead.timerSeconds || 0;
  return (lead.timerSeconds || 0) + Math.floor((Date.now() - lead.timerStartedAt) / 1000);
}
...
function jobRow(lead) {
  return `
    <button class="job-row" type="button" onclick="openLeadDialog('${lead.id}')">
      <span>
        <strong>${escapeHTML(lead.name)}</strong>
...
function leadCard(lead) {
  return `
    <button class="lead-card" type="button" data-id="${lead.id}">
      <strong>${escapeHTML(lead.name)}</strong>
      <small>${escapeHTML(lead.address)}</small>
      <div class="lead-meta">
        <span class="chip">${currency(lead.price)}</span>
...
function scheduledJobsForSlot(date, hour) {
  return getActiveLeads()
    .filter((lead) => lead.scheduledDate === date && lead.scheduledTime && Math.floor(timeToMinutes(lead.scheduledTime) / 60) === hour)
